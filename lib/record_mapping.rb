...
require_dependency File.expand_path(File.join('..', 'record_mapping', 'association_unit'), __FILE__)
...
# 默认不映射 created_at, updated_at
    ...
    def mapping_override(new_mapping)
      ...
      new_mapping.each { |name, options|
        ...
        unit_options = {:name => name.to_s}.merge(options)
        if unit
          unit.override(unit_options)
        else
          if unit_options[:association]
            @unit_pool << AssociationUnit.new(
                {
                    :type => :accessor,
                    :record_class => @record_class,
                    :container => @container
